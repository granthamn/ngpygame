...
class Langton:
    ...
    def drawSteps(self):
        stepsSize = self.antFont.size("Steps: " + str(self.antSteps))
        dirSize = self.antFont.size("DIR: " + str(self.theAnt.getDir()))
        self.stepSurf = pg.Surface(stepsSize)
        self.dirSurf = pg.Surface(dirSize)
        ...
        self.stepSurf.fill(WHITE)
        self.dirSurf.fill(WHITE)
        self.stepSurf.blit(self.stepsText,(0,0))
        self.dirSurf.blit(self.dirText,(0,0))
        self.DS.blit(self.stepSurf,(50,20))
        self.DS.blit(self.dirSurf,(50,40))
    ...
    def mainLoop(self):
        ...
        while not self.quit:
            ...
            antX = self.theAnt.getX()
            antY = self.theAnt.getY()
            antCell = self.dispBuff[antY][antX]
            if self.dispBuff[antY][antX] == 0:
                self.cellColour = BLACK
            else:
                self.cellColour = WHITE

            self.antRect = pg.Rect(antX*PIX_SIZE,antY*PIX_SIZE,PIX_SIZE,PIX_SIZE)
