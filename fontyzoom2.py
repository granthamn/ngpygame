...
while not quitIt:
    ...
    for fonty in fontyList:
        ...
        if fonty[2] < maxFonty and fonty[2] > minFonty:
            if fonty[3] == FONTYDIR.FONTYUP:
                fonty[2] += fontyStep
                (fx,fy) = (fonty[0][0],fonty[0][1]+2)
                fonty2 = [(fx,fy),fonty[1],fonty[2],fonty[3],fonty[4]]
                fontyList.remove(fonty)
                fontyList.append(fonty2)
            else:
                fonty[2] -= fontyStep
    ...
    screen.blit(DS,(0,0))    
    pg.display.flip()
    if len(fontyList) > 100:
        fontyList.remove(fontyList[99])
