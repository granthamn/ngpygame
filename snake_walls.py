...
class Menu:
    ...
    def draw(self, DS):
        ...
        DS.blit(self.quitText, (200, 260))
        DS.blit(self.speedText, (200, 240))
...
class SnakeGame:
    def __init__(self, caption):
        ...
        self.fruit = Segment(SEGTYPE.FRUIT,BLUE, 12, 4)
    ...
    def initVars(self):
        ...
        if self.mode == MODE.HARD:
            self.wallList = [(x,y) for y in range(5,16) for x in [5,14]]
        else:
            self.wallList = []
    ...
    def doUpdates(self):
        ...
        self.snake.updatePos(self.wallList,self.mode)
        if self.snake.getStatus() != SNAKESTAT.DEAD:
            if self.snake.getCoord() == self.fruit.getCoord():
                self.snake.grow()
                # Scoreis based on time between fruits and length of snake
    ...
    def chooseFruitLoc(self):
        ...
        while not validFruitLoc:
            ...
            if ((tempX, tempY) not in self.snake.getPos() or self.snake.getPos()[(tempX, tempY)] == 0) and (self.mode == MODE.EASY or (tempX,tempY) not in self.wallList):
                validFruitLoc = True
        return (tempX, tempY)
    ...
    def drawWalls(self):
        for wallCoord in self.wallList:
            self.wallRect = pg.Rect(wallCoord[0]*PIX_WIDTH,wallCoord[1]*PIX_HEIGHT,PIX_WIDTH,PIX_HEIGHT)
            pg.draw.rect(self.DS,WHITE,self.wallRect)
...
class Snake:
    def __init__(self,dir):
        ...
        self.dir = dir # 1 left, 2 up ,3 right, 4 down
        self.segList = []
    ...
    def updatePos(self,wallList,mode):
        ...
        self.coOrd = (self.xPos, self.yPos)
        if (self.coOrd in self.posDict and self.posDict[self.coOrd] == 1) or (mode == MODE.HARD and self.coOrd in wallList):
